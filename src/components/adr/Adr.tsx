...
export default function Adr({
  ...
  useEffect(() => {
    ...
    chartRefCurentADR.current = adrChart;

    return () => {
      adrChart.remove();
      seriesRef.current = null;
      chartAdr.current = null;
      chartRefCurentADR.current = null;
    };
  }, []);
  ...
  useEffect(() => {
    // tính ADR
    const adrData = calcADR(candleData, dataCurrent.period)
      .filter((d) => d.adr !== null)
      .sort((a: any, b: any) => a.time - b.time);

    const dataNew = adrData.map((d) => ({
      time: d.time as UTCTimestamp, // UNIX time (giây)
      value: d.adr,
    }));
    ...
    <>
      <div ref={chartAdrRef} style={{ position: 'relative', marginTop: '4px' }}>
        ...
        <div className="absolute left-4 top-1 text-[12px]">
          ADR {dataCurrent.period} <span className="text-gray-400">SMA</span>{' '}
          <span className="text-[var(--color-background)] ml-2">{currentAdr && currentAdr.adr.toFixed(2)}</span>
        </div>
      </div>
