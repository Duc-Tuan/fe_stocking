    ...
    return (
        <div className='grid grid-cols-5 gap-2 h-[calc(100vh-95px)]'>
            <div className="col-span-1 lg:col-span-1 shadow-md shadow-gray-500 rounded-lg p-2 relative" ref={containerRef}>
                ...
                />
                {data.map((d, idx) => {
                    return <Button id="button-history" onClick={() => handlelick(d, idx, d.active)} key={d.type} className={`${d.active ? "text-white" : "text-[var(--color-background)] hover:bg-[var(--color-background-opacity-2)]"} cursor-pointer w-full text-left block shadow-none p-2 mb-1 text-[12px] md:text-[16px]`}>{t(d.title)}</Button>
                })}
            </div>
            <div className="col-span-4 lg:col-span-4 shadow-md shadow-gray-500 rounded-lg p-4 overflow-y-scroll my-scroll">
