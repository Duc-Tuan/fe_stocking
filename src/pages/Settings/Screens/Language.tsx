...
const ChangColor = ({ t }: any) => {
  ...
  return <div className="mb-2" ref={languageRef}>
    ...
    <div className={`flex justify-start items-center gap-2 overflow-hidden transition-all duration-300 shadow-md shadow-gray-200 ${isOpen ? "max-h-40 p-3" : "max-h-0"}`}>
      {dataColor.map((a) =>
        <Button key={a.value} onClick={() => handleLanguage(a.value)} className={`${a.label} flex justify-center items-center w-[36px] h-[36px] cursor-pointer shadow-md shadow-gray-300`}>
...
const LanguageComponent = ({ t }: any) => {
  ...
  return <div className="" ref={languageRef}>
    ...
    <div
      className={`overflow-hidden transition-all duration-300 shadow-md shadow-gray-200 ${isOpen ? "max-h-40" : "max-h-0"
        }`}
    >
      {dataLanguage.map((a) =>
