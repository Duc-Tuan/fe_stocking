...
export default function AllLot() {
  ...
  const handleFilter = (data: IFilterAllLot) => {
    setQuery((prev) => {
      return {
        ...
        page: 1,
        statusType: data.status ? 'RUNNING' : undefined,
      };
    });
  };
  ...
  return (
    <div>
      <Filter
        subButton={<TaskSquare setDataLost={setData} query={query} idx={data.map((a) => a.id)} isClose={isClose} />}
        ...
        {!loading ? (
          data.length === 0 ? (
            ...
            data.map((a, idx) => (
              <div
                key={idx}
                className="shadow-sm shadow-gray-300 p-4 pt-2 mb-2 bg-[var(--color-background-opacity-1)] text-[12px] md:text-[16px]"
              >
                  ...
                  <div className="flex justify-start items-center gap-2">
                    ...
                    {a.type === 'RUNNING' && a.status !== 'Lenh_thi_truong' && (
                      <TooltipNavigate
                        ...
                        className="ml-1 shadow-sm w-[30px] md:w-[30px] h-[30px] md:h-[30px] p-0 flex justify-center items-center"
                    ...
                    {a.type === 'RUNNING' && a.status === 'Lenh_thi_truong' && (
                      <TooltipNavigate
                        ...
                        classSub="ml-1 shadow-sm w-[26px] h-[26px] md:w-[28px] md:h-[28px] p-0 flex justify-center items-center"
                        iconName="icon-edit-lot"
                        path="#"
                ...
                <div className="md:grid md:grid-cols-2 mt-3">
                  ...
                  <div className="flex justify-start items-center gap-2">
                    ...
                    <div className="flex justify-start items-center gap-1">
                      ...
                      <Icon
                        name="icon-chart-transaction"
                        className="text-[var(--color-background)] mt-[2px]"
                        width={18}
                        height={18}
                      />
                    </div>
                  ...
                  <div className="flex justify-start items-center gap-2">
                    <span>{t('Điều kiện thêm')}:</span>
                    <span className="font-semibold">{a?.IsUSD ? t('Áp dụng') : t('Không áp dụng')}</span>
                  </div>
                  {a.IsUSD && (
                    <div className="flex justify-start items-center gap-2">
                      <span>{t('Số tiền lỗ chấp nhận')}(usd): </span>
                      <span className="font-semibold">{a?.usd}usd</span>
                    </div>
                  )}
                  <div className="w-full col-span-2">
                    <div className="flex justify-between items-center">
                      <span>{t('Thông tin chi tiết cặp tiền vào lệnh')}: </span>
                      {colorbg(a.status).label !== 'Đang chờ' && a.type === 'RUNNING' && (
                        <TooltipNavigate
                          handle={() => {
                            setDataCurrent({ ...a, lot: idx + 1 });
                            setOpen((prev) => !prev);
                          }}
                          classSub="shadow-sm w-[26px] md:w-[28px] h-[26px] md:h-[28px] p-0 flex justify-center items-center"
                    ...
                    <div className="mt-2">
                      <div className="flex justify-between items-center">
                        <div className="border flex-1 border-gray-200">
                          <div className="text-[12px] md:text-sm font-bold text-center border-b border-b-gray-200 p-1">
                            {t('Tên cặp tiền')}
                          </div>
                          {a.bySymbol.map((d, idx) => (
                            <div key={idx} className="text-[12px] md:text-sm text-center font-semibold h-6">
                              {d.symbol}
                            </div>
                          ))}
                        </div>
                        <div className="border flex-1 border-gray-200">
                          ...
                          {a.bySymbol.map((d, idx) => (
                            <div
                              key={idx}
                              className={`text-center h-6 text-[12px] font-bold flex justify-center items-center ${colorTextType(
                                d.type,
                              )}`}
                            >
                              {d.type}
...
const TaskSquare = ({
  ...
  query: QueryLots;
  idx: number[];
  setDataLost: Dispatch<SetStateAction<IHistoryLot[]>>;
  isClose: boolean;
}) => {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  ...
  const changeValue = (value: ChangeEvent<HTMLInputElement>, title: 'to' | 'from') => {
    ...
    if (title === 'from') {
      return setData((prev) => ({
        ...prev,
        from:
          Number(v) <= Number(data.to ?? query.total)
            ? Number(v) < 0
  ...
  return (
    <div className="inline-block">
      {visible ? (
        <div
          className={`md:relative absolute top-full  bg-white left-0 text-[12px] md:text-sm ml-4 shadow-md rounded-md py-1 px-2 shadow-gray-300 flex justify-start items-center gap-2 transition-all duration-200 ${
        ...
        >
          ...
          <input
            ...
            value={data.from}
            placeholder="1"
            className="w-16 text-center border border-[var(--color-background)] rounded-sm font-bold px-1 py-0.5 appearance-none focus:outline-none focus:ring-0 focus:border [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [MozAppearance:textfield]"
          />
          <span>{t('đến')}</span>
          <input
            type="number"
            ...
            className="w-16 text-center border border-[var(--color-background)] rounded-sm font-bold px-1 py-0.5 appearance-none focus:outline-none focus:ring-0 focus:border [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [MozAppearance:textfield]"
          />
          <Button onClick={handleToggle} className="ml-6 h-[30px] text-red-500 border p-0 px-2 cursor-pointer">
            {t('Hủy')}
          </Button>
          <Button
            onClick={() => {
              !isClose &&
                toast.error(t('Hiện không có lô nào được đưa lên thị trường nên thể thực hiện chức năng này'));
              isClose && setOpenModal((prev) => !prev);
            }}
            className={`${
              isClose ? 'bg-[var(--color-background)]' : 'bg-gray-300'
            } h-[30px] text-white border p-0 px-2 cursor-pointer`}
          >
            {t('Xác nhận')}
          </Button>
        </div>
        ...
        <TooltipNavigate
          handle={handleToggle}
          iconName="icon-task-square"
          path="#"
          title="Chốt lệnh nhanh"
          className="ml-2 w-[30px] md:w-[36px] h-[30px] md:h-[36px] flex justify-center items-center"
        />
      )}
...
const ModalTaskSquare = ({
  ...
  data: { to: string | undefined; from: string | undefined };
}) => {
  const { t } = useTranslation();
  const [loading, setLoading] = useState<boolean>(false);
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {t(
                        'Nếu bạn xác nhận đóng lệnh nhanh thì sẽ không thể nào mở lại được tại thời điểm này nữa. Bạn vẫn muốn xác nhận!',
                      )}
                    </p>
                  </div>
                </div>
...
const Modal = ({
  ...
  open: boolean;
  setOpen: Dispatch<SetStateAction<boolean>>;
  dataCurrent: IActiveHistoryLot | null;
}) => {
  const { t } = useTranslation();
  const [loading, setLoading] = useState<boolean>(false);
          ...
          >
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <DialogTitle as="h3" className="text-base font-semibold text-gray-900">
                    {t('Bạn xác nhận muốn đóng lệnh của lô')} {dataCurrent?.lot} {t('dãy')}{' '}
                    {dataCurrent?.bySymbol.map((a) => a.symbol + ' ')}
                  </DialogTitle>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {t(
...
const ModalDelete = ({
  ...
  const handleClick = async () => {
    ...
    setLoading(true);
    await deleteLot(dataCurrent?.id)
      .then(() => {
        setOpen(false);
        setLoading(false);
        setDataLost((prev) => {
              ...
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {t(
                        'Nếu bạn xác nhận lô này thì các cặp tiền cũng trong lô cũng sẽ xóa theo. Bạn vẫn muốn xác nhận chứ!',
                      )}
                    </p>
                  </div>
                </div>
...
const ModalUpdate = ({
  ...
  useEffect(() => {
    setDataInput({
      stop_loss: String(dataCurrent?.stop_loss),
      take_profit: String(dataCurrent?.take_profit),
    });
  }, [dataCurrent]);

  const handleClick = async () => {
    if (!dataCurrent) return;
    if (dataInput.stop_loss === '' || dataInput.take_profit === '')
      return toast.error(t('Vui lòng nhập đầy đủ thông tin.'));
    setLoading(true);
    await patchLot({
      id: dataCurrent.id,
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left  flex-1">
                  ...
                  <div className="mt-2">
                    <label htmlFor="" className="text-[12px] md:text-base">
                      {t('Cắt lỗ')}: (PNL)
                    </label>
                    <input
                      className="text-[12px] md:text-base border p-2 w-full rounded-sm focus:border-[var(--color-background)] focus:outline-hidden border-gray-300 mb-2"
                    ...
                    <input
                      className="text-[12px] md:text-base border p-2 w-full rounded-sm focus:border-[var(--color-background)] focus:outline-hidden border-gray-300"
                      placeholder={t('Nhập thông tin...')}
