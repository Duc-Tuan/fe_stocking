...
import {
  ...
  dataSelectTypeAcc,
  type IActivateTypetransaction,
  type IOptionTypeAcc,
  type IOrderTransaction,
  type IStatus_sl_tp,
} from './type';
...
export default function TransactionPage() {
  ...
  useEffect(() => {
    ...
    setDatAccTransaction(dataNew);
  }, [dataServerTransaction, selectTypeAcc]);

  const [activateTypetransaction, setActivateTypetransaction] =
    useState<IActivateTypetransaction[]>(dataActivateTypetransaction);
  ...
  useEffect(() => {
    if (activateTypetransaction.find((i) => i.active)?.type === 'Lenh_thi_truong') {
      setPnl(currentPnl?.total_pnl ?? 0);
    }
  }, [currentPnl, activateTypetransaction]);

  const isCheckOrther =
    selectTypeAcc.find((a) => a.actiavte)?.value === 'USD' ||
    selectTypeAcc.find((a) => a.actiavte)?.value === 'COPY' ||
    selectTypeAcc.find((a) => a.actiavte)?.value === 'DEPOSIT';
      ...
      <div className="w-full max-w-4xl p-0 md:p-4 space-y-4 mt-10 md:mt-0">
          ...
          <div className="row-start-2 md:row-start-1 col-span-4 md:col-span-2 shadow-xs shadow-gray-500 rounded-lg p-3 space-y-2">
            ...
            <div className="flex flex-col justify-between items-center w-full h-[calc(100%-40px)]">
              <div className="w-full">
                {activateTypetransaction.filter((a) => a.active && a.type === 'Lenh_thi_truong').length === 0 && (
                  <div className="text-sm flex justify-between py-2">
                    <span className="font-bold text-[12px] md:text-sm">{t('Giá')}: (PNL)</span>
                ...
                <div className="text-sm flex justify-between items-center pb-2">
                  <span className="font-bold text-[12px] md:text-sm">{t('Cắt lỗ')}: (PNL)</span>
                  <div className="flex items-center space-x-1">
                    <button
                ...
                <div className="text-sm flex justify-between items-center pb-2">
                  <span className="font-bold text-[12px] md:text-sm">{t('Chốt lời')}: (PNL)</span>
                  <div className="flex items-center space-x-1">
                    <button
              ...
              <div className="w-full text-sm flex justify-between items-center border-t border-t-gray-300 border-solid pt-2">
                <span className="font-bold text-[12px] md:text-sm">{t('Hết hạn')}</span>
                <span>GTC</span>
              </div>
            </div>
          </div>
                  ...
                  >
                    <Button
                      ...
                      className={`flex w-full justify-between items-center  ${
                        i === 0 ? 'rounded-none border-b border-b-gray-300 border-solid py-1 px-2' : 'py-2 px-2'
                      } cursor-pointer shadow-none hover:bg-[var(--color-background-opacity-2)] transition`}
                    >
                      <div className={`text-[10px] md:text-sm font-bold ${a.color}`}>{t(a.title)}</div>
                      {a.active && (
                        <span>
                          <Icon name="icon-check" width={18} height={18} className="text-[var(--color-background)]" />
                        </span>
                      )}
                    </Button>
                  </TooltipCustom>
        ...
        {activateTypetransaction.filter((a) => a.active && a.type === 'Lenh_thi_truong').length === 0 ? (
          <Button
            ...
            className={`${
              isCheckSumbit ? 'bg-[var(--color-background)]' : 'bg-gray-300'
            } p-0 w-full h-10 font-bold cursor-pointer text-white px-2 shadow-xs shadow-gray-500 text-[12px] md:text-sm`}
          >
          ...
          <div className="flex gap-2 h-10">
            <Button
              onClick={() => {
                (currentPnl?.total_pnl ?? 0) > stopLoss &&
                  (currentPnl?.total_pnl ?? 0) < takeProfit &&
                  handleSendOrder('Xuoi_Limit');
              }}
              aria-current="page"
              className={`${
                (currentPnl?.total_pnl ?? 0) > stopLoss && (currentPnl?.total_pnl ?? 0) < takeProfit
                  ? 'bg-rose-600 hover:bg-red-600'
                  : 'bg-gray-300'
              } p-0 flex-1  text-white rounded-lg shadow-xs shadow-gray-500  cursor-pointer font-bold text-[12px] md:text-sm`}
            >
              {t('Xuôi')}
            </Button>
            <Button
...
const Modal = ({
  ...
  const handleClick = async () => {
    setLoading(true);
    await postSendOrder(dataCurrent)
      .then((data) => {
        setLoading(false);
                ...
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2 grid grid-cols-2">
                    <div className="flex col-span-1">
                      ...
                      <span className="ml-1 font-semibold text-[var(--color-background)]">
                        {dataCurrent?.account_monitor_id}
                      </span>
                    </div>
                    <div className="flex col-span-1">
                      <span>{t('Tk giao dịch')}:</span>
                      <span className="ml-1 font-semibold text-[var(--color-background)]">
                        {dataCurrent?.account_transaction_id}
                      </span>
                    </div>
                    <div className="flex col-span-2">
                      <span>{t('Trạng thái')}:</span>
                      <span className="ml-1 font-semibold text-[var(--color-background)]">
                        {t(String(dataActivateTypetransaction.find((a) => a.type === dataCurrent?.status)?.title))}
                      </span>
                    </div>
                    <div className="flex col-span-1">
                      <span>{t('Giá')}:</span>
                    ...
                    <div className="flex col-span-1">
                      <span>SL:</span>
                      <span className="ml-1 font-semibold text-[var(--color-background)]">
                        {dataCurrent?.stop_loss}
                      </span>
                    </div>
                    <div className="flex col-span-1">
                      <span>TP:</span>
                      <span className="ml-1 font-semibold text-[var(--color-background)]">
                        {dataCurrent?.take_profit}
                      </span>
                    </div>
                    {dataCurrent?.IsUSD && (
                      <>
                        <div className="flex col-span-1">
                          ...
                          <span className="ml-1 font-semibold text-[var(--color-background)]">
                            {dataCurrent?.IsUSD ? t('Áp dụng') : t('Không áp dụng')}
                          </span>
                        </div>
                        <div className="flex col-span-1">
                          <span>{t('Số tiền lỗ chấp nhận')}:</span>
                          <span className="ml-1 font-semibold text-[var(--color-background)]">
                            {dataCurrent?.usd}usd
                          </span>
                        </div>
                      ...
                      <div className="mt-2">
                        <div className="flex justify-between items-center">
                          <div className="border flex-2 border-gray-200">
                            <div className="text-sm font-bold text-center border-b border-b-gray-200 p-1">
                              {t('Tên cặp tiền')}
                            </div>
                            {dataCurrent?.by_symbol?.map((d, idx) => (
                              <div key={idx} className="text-sm text-center font-semibold h-6">
                                {d.symbol}
                              </div>
                            ))}
                          </div>
                          <div className="border flex-1 border-gray-200">
                            ...
                            {dataCurrent?.by_symbol?.map((d, idx) => (
                              <div
                                key={idx}
                                className={`text-center h-6 text-[12px] font-bold flex justify-center items-center ${
                                  d.type === 'SELL' ? 'text-red-500' : 'text-blue-500'
                                }`}
                              >
                                {d.type}
                              </div>
                          ...
                          <div className="border flex-1 border-gray-200">
                            ...
                            {dataCurrent?.by_symbol?.map((d, idx) => (
                              <div key={idx} className="text-sm text-center h-6">
                                {d.current_price?.toFixed(5)}
                              </div>
                            ))}
                          </div>
                        </div>
